#!/bin/bash
USG="Usage:\n$0 <htmsrc> <htmbin> <assets>"
echo "ShelfHTM/1.0 Shelf-CompSH/1.0"
echo
echo
[ x$1 == x ] && echo -e $USG && exit 1
[ x$2 == x ] && echo -e $USG && exit 1
[ x$3 == x ] && echo -e $USG && exit 1
if [ ! -d $1 ];then echo -e $USG;exit 1;fi
if [ ! -d $2 ];then echo -e $USG;exit 1;fi
if [ ! -d $3 ];then echo -e $USG;exit 1;fi
echo "Compiling..."
mkdir .sbs
echo "" | base64 -d > .sbs/buildhelper.sh
echo "TEVOPWBlY2hvICR7IzJ9ICsgMSB8IGJjYApYPWBlY2hvICQxIHwgY3V0IC1jICR7TEVOfS1gCmNwICQyLyRYICQzLyRYCmNkICQ0CmZpbmQgKiAtdHlwZSBmIC1leGVjIGJhc2ggLi4vLnNicy9idWlsZGhlbHBlci5zaCB7fSAuLi8kMi8kWCAuLi8kMy8kWCAiOyIKY2QgLi4K" | base64 -d > .sbs/file.sh
find $1 -type f -exec bash .sbs/file.sh {} $1 $2 $3 ";"
rm -rdf .sbs
echo "Finished."
